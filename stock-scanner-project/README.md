...
- **Tab 组件第一次被访问后会一直保持挂载**：`App` 维护一个 `visitedTabs` 集合，切换 Tab 时把目标 Tab 加进去；渲染时只对"访问过的 Tab"逐个渲染组件，用 CSS `display: none` 隐藏未激活的那些，而不是只渲染当前激活的一个。这是为了修一个真实体验问题——之前每次切换 Tab 都会把上一个 Tab 的组件整个卸载再重新挂载，导致它的本地状态（已加载的股价、走势图数据）被清空，即使 localStorage 里的缓存还新鲜，切回来的时候卡片也会先花一下再重新显示，看着像是又发了一遍请求。现在一个 Tab 只在第一次被点开时挂载一次，之后来回切换既不会有这个"重新加载"的闪烁，也不会因为反复访问而触发额外的网络请求；没打开过的 Tab 也不会被提前挂载、提前发请求
